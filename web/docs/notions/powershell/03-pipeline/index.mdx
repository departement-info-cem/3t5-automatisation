...
![Principe de la boîte noire](boitenoire.svg)
...
![Le chainage de commandes](chainage.svg)
...
```text
PS C:\> Get-Item "C:\Minou\ListeDeChats.txt" | Get-Content
Garfield
Salem
Félix
Azrael
...
<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> $LesChats = Get-Item "C:\Minou\ListeDeChats.txt" | Get-Content
PS C:\> $LesChats
Garfield
Salem
Félix
...
La sélection ne "transforme" pas vraiment l'objet, elle en produit plutôt une copie transformée. Par exemple, `Select-Object *` renvoie une version modifiée de l'objet où toutes les propriétés sont visibles.

<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> Get-Service -Name "wuauserv" | Select-Object *

UserName            : LocalSystem
Description         : Active la détection, le téléchargement et l’installation...
...
Par exemple, la commande suivante permet de "transformer" l'objet représentant le service "wuauserv" en objet composé uniquement de ses propriétés `Name`et `Status`. Notez que vous pouvez omettre `Property` puisqu'il s'agit d'un **paramètre positionnel**. 

<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> Get-Service -Name wuauserv | Select-Object -Property Name, Status
...
On remarque qu'à l'exécution de cette commande, il reste des entêtes de colonnes. Alternativement, on peut extraire seulement la **valeur** de la propriété en spécifiant le paramètre `ExpandProperty`. Ce paramètre n'est pas positionnel contrairement au paramètre `Property`, on est donc obligé de le spécifier. Sinon, on aura des en-têtes de colonnes.

<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> Get-Service -Name wuauserv | Select-Object Status

 Status
 ------
Running

PS C:\> Get-Service -Name wuauserv | Select-Object -ExpandProperty Status
Running
```
</ConsoleWindow>
...
<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> Get-Service | Format-Table Name, Status

Name                                      Status
----                                      ------
AarSvc_87709                             Stopped
ADPSvc                                   Stopped
...
<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> Get-Service | Format-List Name, DisplayName, BinaryPathName, Status, StartType

Name           : AarSvc_87709
DisplayName    : AarSvc_87709
BinaryPathName : C:\WINDOWS\system32\svchost.exe -k AarSvcGroup -p
Status         : Stopped
StartType      : Manual

Name           : ADPSvc
DisplayName    : Service de plateforme de données agrégées
...
<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> Start-Process -FilePath "C:\Windows\Notepad.exe"
```
</ConsoleWindow>
...
<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> Start-Process -FilePath "C:\Windows\Notepad.exe" -PassThru

 NPM(K)    PM(M)      WS(M)     CPU(s)      Id  SI ProcessName
 ------    -----      -----     ------      --  -- -----------
     12     2,86      11,80       0,03   26988   1 Notepad
```
</ConsoleWindow>
...
<ConsoleWindow backgroundColor="#000046" noCopy="true">
```text
PS C:\> Get-Service -Name BITS | Tee-Object -Variable "MaVariable" | Format-List
...
------   ----               -----------
Running  BITS               Service de transfert intelligent en a…
```
</ConsoleWindow>
