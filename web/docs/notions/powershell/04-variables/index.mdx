...
<ConsoleWindow title="PowerShell" backgroundColor="#000046" noCopy="true">
```text
PS C:\> $CrisDAnimaux = "Miaou,Wouf,Cocorico"
PS C:\> $CrisDAnimaux
Miaou,Wouf,Cocorico
PS C:\> $CrisDAnimaux.Length
19
PS C:\> $CrisDAnimaux.ToUpper()
MIAOU,WOUF,COCORICO
PS C:\> $CrisDAnimaux.Split(",")
Miaou
Wouf
Cocorico
```
...
CrisDAnimaux                   Miaou,Wouf,Cocorico
...
| Variable            | Description                                                                       |
| ------------------- | --------------------------------------------------------------------------------- |
| `COMPUTERNAME`      | Le nom d'hôte de l'ordinateur                                                     |
| `USERNAME`          | Le nom de l'utilisateur                                                           |
| `USERDOMAIN`        | Le nom du domaine de l'utilisateur courant (format NetBIOS)                       |
| `USERDNSDOMAIN`     | Le nom du domaine de l'utilisateur courant (format FQDN)                          |
| `LOGONSERVER`       | Le nom du contrôleur de domaine ayant authentifié l'utilisateur lors de son logon |
| `UserProfile`       | Le chemin vers le profil de l'utilisateur (typiquement C:\Users\nomdutilisateur)  |
| `APPDATA`           | Chemin vers le répertoire AppData\\Roaming de l'utilisateur courant               |
| `LocalAppData`      | Chemin vers le répertoire AppData\\Local de l'utilisateur courant                 |
| `TEMP` ou `TMP`     | Le chemin vers le répertoire temporaire de l'utilisateur (même que TMP)           |
| `ALLUSERSPROFILE`   | Le chemin vers le profil utilisateur commun à tous les utilisateurs (programdata) |
| `ProgramData`       | Le chemin du répertoire qui contiennent les configurations du système             |
| `ProgramFiles`      | Le chemin vers le répertoire contenant les fichiers de programme                  |
| `ProgramFiles(x86)` | Le chemin vers le répertoire des fichiers de programme 32-bits (WoW64)            |
| `Path`              | La liste des chemins qui contiennent des commandes exécutables                    |
| `PATHEXT`           | La liste des extensions à considérer comme exécutables                            |
| `PUBLIC`            | Le chemin vers le répertoire public (partagé par tous les profils)                |
| `ComSpec`           | Le chemin vers l'interpréteur de commande par défaut                              |
| `SystemDrive`       | Le lecteur sur lequel Windows est installé (typiquement C:)                       |
| `SystemRoot`        | Le chemin vers le répertoire où Windows est installé (typiquement C:\\Windows)    |
| `PSModulePath`      | La liste des chemins qui contiennent des modules PowerShell                       |
...
<ConsoleWindow title="cmd.exe" backgroundColor="#000000" foregroundColor="#cccccc" noCopy="true">
```text
C:\>echo %appdata%
...
C:\Users\vince\AppData\Roaming>popd
...
PS C:\> $env:APPDATA
C:\Users\vince\AppData\Roaming
PS C:\> Push-Location $env:APPDATA
PS C:\Users\vince\AppData\Roaming> Pop-Location
PS C:\>
```
</ConsoleWindow>
...
<ConsoleWindow title="PowerShell" backgroundColor="#000046" noCopy="true">
```text
PS C:\> [int]"Quarante-Deux"
InvalidArgument: Cannot convert value "Quarante-Deux" to type "System.Int32". Error: "The input string 'Quarante-Deux' was not in a correct format."
```
</ConsoleWindow>
